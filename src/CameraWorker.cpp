...
void CameraWorker::setupCamera()
{
    ...
    if (!capture.isOpened())
    {
        throw std::runtime_error("Failed to open camera " + std::to_string(m_cameraIndex));
    }
}

void CameraWorker::checkCudaAvailability()
{
...
void CameraWorker::setupVideoSettings()
{
...
void CameraWorker::setupSharedMemory()
{
...
void CameraWorker::processFrames()
{
    isRunning = true;
    ...
    while (isRunning)
    {
        try 
        {
            ...
            processFrame(frame, gpuFrame, processedGpuFrame, processedCpuFrame, stream);
...
cv::Mat CameraWorker::captureFrame()
{
    cv::Mat frame;
    ...
    cv::cuda::Stream& stream)
{
    ...
    if (zoomFactor != 1.0)
    {
        processedGpuFrame = applyGpuZoom(processedGpuFrame, frame, stream);
    }
...
cv::cuda::GpuMat CameraWorker::applyGpuZoom(
    const cv::cuda::GpuMat& inputFrame, 
    const cv::Mat& originalFrame, 
    cv::cuda::Stream& stream)
{
    int zoomWidth = originalFrame.cols / zoomFactor;
    int zoomHeight = originalFrame.rows / zoomFactor;

    // Calculate zoom rectangle
    int centerX = originalFrame.cols / 2;
    int centerY = originalFrame.rows / 2;
    cv::Rect zoomRect(
        centerX - zoomWidth / 2,
    ...
    cv::cuda::resize(inputFrame(zoomRect), gpuZoomedFrame, originalFrame.size(), 0, 0, cv::INTER_LINEAR, stream);
    
    return gpuZoomedFrame;
}

void CameraWorker::updateSharedMemory(const cv::Mat& processedCpuFrame)
{
...
void CameraWorker::handleStartupError(const std::exception& ex)
{
    cout << "Startup Exception: " << ex.what() << endl;
    qDebug() << "Startup error:" << ex.what();
    emit errorOccurred(QString("Startup error: %1").arg(ex.what()));
}

void CameraWorker::handleFrameProcessingError(const cv::Exception& e)
{
